...
class DB {
    ...
    public function update_manager_password($username, $pwd) {
        $query = "update t_manager set password = {$pwd} where username = '{$username}'";
        $this->db_query($query);
        //返回多少行收到影响，没有则返回-1
    ...
    public function select_course($message) {
        $query = "select name, credit, start_time, status from t_course where name like '%{$message}%' or number like '%{$message}%'";
        $res = $this->db_query($query);
        while ($row = $res->fetch_array(MYSQLI_ASSOC)) {
            $name = $row['name'];
            $credit = $row['credit'];
            ...
            if ($row['status'] != FALSE) {
                printf("name = %s, credit = %s, start_time = %s.<br />", $name, $credit, $startTime);
            }
        }
